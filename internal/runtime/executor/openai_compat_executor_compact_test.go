...
func TestOpenAICompatRequestURL(t *testing.T) {
	tests := []struct {
		name     string
		baseURL  string
		endpoint string
		...
		{
			name:     "append endpoint from v1 base",
			baseURL:  "https://example.com/v1",
			endpoint: "/chat/completions",
			want:     "https://example.com/v1/chat/completions",
		},
		{
			name:     "keep base when same endpoint already present",
			baseURL:  "https://example.com/v1/chat/completions",
			endpoint: "/chat/completions",
			want:     "https://example.com/v1/chat/completions",
		},
		{
			name:     "trim trailing slash when same endpoint present",
			baseURL:  "https://example.com/v1/chat/completions/",
			endpoint: "/chat/completions",
			want:     "https://example.com/v1/chat/completions",
		},
		{
			name:     "swap endpoint from chat to compact",
			baseURL:  "https://example.com/v1/chat/completions",
			endpoint: "/responses/compact",
			want:     "https://example.com/v1/responses/compact",
		},
		{
			name:     "swap endpoint from compact to chat",
			baseURL:  "https://example.com/v1/responses/compact",
			endpoint: "/chat/completions",
			want:     "https://example.com/v1/chat/completions",
		},
		{
			name:     "swap endpoint from completions to chat",
...
func TestOpenAICompatExecutorChatBaseURLEndpointPassthrough(t *testing.T) {
	var gotPath string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		w.Header().Set("Content-Type", "application/json")
...
func TestOpenAICompatExecutorCompactBaseURLEndpointPassthrough(t *testing.T) {
	var gotPath string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		w.Header().Set("Content-Type", "application/json")
	...
	exec := NewOpenAICompatExecutor("openai-compatibility", &config.Config{})
	auth := &cliproxyauth.Auth{Attributes: map[string]string{
		"base_url": server.URL + "/v1/responses/compact",
		"api_key":  "test",
	}}
